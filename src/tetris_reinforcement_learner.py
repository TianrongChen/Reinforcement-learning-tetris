...
class TetrisReinforcementLearner:
    def __init__(self, tetris_game):
        ...
        self.CEM_weights=defaultdict(lambda:0)
    def train(self):
        self.play()
    ...
    def get_top_q_value_pair(self, state, action_sequences):
        ...
        top_lookahead_q_value_pairs = self.get_top_n_q_value_pairs(state, lookahead_sequences, 5)
        # print(top_lookahead_q_value_pairs)
    ...
    def get_successor_state_reward(self, state, action_sequence):
        ...
        reward=self.drop_stone1(successor_state)
        # print('successor_state_is',successor_state)
        return reward

    def get_next_action_reward(self,state,action):
        board=state['board']
        next_state_reward=self.get_successor_state_reward(state,action)
    ...
    def get_holes_number(self,state):
        board=state["board"]
        holes_num=0
        for i in range(len(board)):
            row=board[i]
            if row.count(0)!=len(row):
                holes_num=holes_num+row.count(0)
        return holes_num
    ...
    def join_action_sequence(self, action_sequence):
        ...
        # print(two_action_sequences_happened)
    ...
    def update(self, state, action_sequence, new_state, reward):
        ...
        # print("old_state_value:",old_state_value)
        ...
        for feature, value in features.items():
            ...
            new_weight =  old_weight + self.alpha * new_value
